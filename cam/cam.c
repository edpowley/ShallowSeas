...
#include	<stdio.h>
#include	<math.h>
#include	"vogl.h"	/*vogl file to be placed in current directory	*/
#include	"vodevice.h"	/*vogl file to be placed in current directory	*/
...
#define		nspp	2	/*number of species: indexed 0,1		*/
#define		nstage	3	/*maximum number of life stages: indexed 0,1,2 	*/
#define		tmax	1000.0	/*time at which to end simulation		*/
#define		dt	1.0	/*time step					*/
#define		xmax	10	/*number of cells along x coordinate		*/
#define		ymax	10	/*number of cells along y coordinate		*/
#define		nbrmax	4	/*number of neighbour cells			*/


/********************************************************************************
* Switches for turning on and off parts of the program				*
********************************************************************************/
#define method_leslie	2	/* Method for birth-death matrices
				   0: transition matrices fixed
				   1: transition matrices with feedbacks	
				   2: transition matrices with time-scaling	*/

#define method_move	0	/* Method for movement between cells
				   0: constant proportion move			*/

#define	method_vogl	1	/* vogl graphics
				   0: off
				   1: on					*/
...
struct	species_properties
	{	float	N[nstage][xmax][ymax];		/*    species densities by life stage and grid cell	*/
		float	newN[nstage][xmax][ymax];	/*new species densities by life stage and grid cell	*/
		float	sumN[nstage];			/*    species densities by life stage summed over grid	*/
		float	leslie[nstage][nstage];		/*matrix for dynamics -- Leslie-matrix-like		*/
		float	move[2];			/*vector for movement between 2 cells			*/
	};

struct	species_properties	species[nspp];

struct	community_properties
	{	float	alpha[nspp][nspp];		/*  interaction matrix of species			*/
		float	N[nstage][xmax][ymax];		/*  community densities by life stage and grid cell	*/
	};

struct	community_properties	community;
...
/********************************************************************************
* Variables for switching on bits of output for checking			*
********************************************************************************/
int	flag_leslie = 1;	/*Info on transition matrices:  0:off  1:on	*/


/********************************************************************************
* Files with extra procedures							*
********************************************************************************/
#include	"cam.leslie.c"		/*Procedures for birth and deaths	*/
#include	"cam.move.c"		/*Procedures for movement		*/
#include	"cam.vogl.c"		/*Procedures for vogl graphs		*/
...
{
	community.alpha[0][0] = 1.0;
	community.alpha[0][1] = 0.5;
	community.alpha[1][0] = 0.5;
	community.alpha[1][1] = 1.0;
}
...
/****************************************************************************************************************
* This procedure organizes output for each iteration.								*
****************************************************************************************************************/
void	output(t)
float	t;
{
    ...
    /***Output species information on grid***/
	out = fopen("output.grid.dat", "a");
	for (sp=0; sp<nspp; ++sp)
	{	for (i=0; i<nstage; ++i)
		{	fprintf(out, "Grid:  time:%6.4f  species:%2d life stage:%2d   total:%7.4f\n\n", t, sp, i, species[sp].sumN[i]);
			for (x=0;  x<xmax;   ++x)
			{	for (y=0;  y<ymax;   ++y)	fprintf(out, "%6.2f ", species[sp].N[i][x][y]);
				fprintf(out, "\n");
			}
			fprintf(out, "\n");
		}
    ...
    /***Output community information on grid***/
	for (i=0; i<nstage; ++i)
	{	fprintf(out, "Grid:  time:%6.4f  community   life stage:%2d\n\n", t, i);
		for (x=0;  x<xmax;   ++x)
		{	for (y=0;  y<ymax;   ++y)	fprintf(out, "%6.2f ", community.N[i][x][y]);
			fprintf(out, "\n");
		}
		fprintf(out, "\n");
	}
...
float	t;
{
	...
	for (nbr=0; nbr<nbrmax; ++nbr)
	{
		...
		neighbour_coords(x, y, nbr, &xnbr, &ynbr);
